...
sys.path.append(os.path.join(base_dir, "drone_based_RGB/treesat_benchmark_genus"))
sys.path.append(os.path.join(base_dir, "drone_based_RGB/treesat_benchmark_species"))
import warnings
warnings.filterwarnings("ignore")
from drone_based_RGB.treesat_benchmark_genus.TreeSat_Benchmark.trainers.metrics import *
from drone_based_RGB.treesat_benchmark_genus.TreeSat_Benchmark.trainers.augmenter import Augmenter
from drone_based_RGB.treesat_benchmark_genus.TreeSat_Benchmark.trainers.dataloaders import get_dataloader
from drone_based_RGB.treesat_benchmark_genus.TreeSat_Benchmark.models import get_classification_model
from drone_based_RGB.treesat_benchmark_genus.TreeSat_Benchmark.trainers.basetrainer import ModelTrainer
from drone_based_RGB.treesat_benchmark_genus.TreeSat_Benchmark.trainers.utils import get_class_weights, set_up_unfrozen_weights
from drone_based_RGB.treesat_benchmark_genus.TreeSat_Benchmark.trainers.transforms import xform_aer_scratch, xform_aer_3bands_scratch
...
classes_withno_minor = ["Abies",
                ...
                "Pseudotsuga",
                "Quercus"]
classes_translated_withno_minor = ["Fir",
           "Maple",
           "Alder",
           "Birch",
           "Cleared",
...
classes = classes_withno_minor
classes_translated = classes_translated_withno_minor


path_config = os.path.join(base_dir, "treesat_benchmark_genus/configs/aer_only/aerial_cyclic_Families_v8_3Bands_no_NIR_Scratch.json")
...
model.load_state_dict(torch.load(
    os.path.join(base_dir, "..", "models/drone_rgb_model_genus.pt"),
...
COLOR_SHADERS = {
    1: (1.0, 0.5, 0.5),  # Red tint
    2: (0.5, 1.0, 0.5),  # Green tint
    3: (0.5, 0.5, 1.0),  # Blue tint
    4: (1.0, 1.0, 0.5),  # Yellow tint
    5: (1.0, 0.5, 1.0),  # Magenta tint
    6: (0.5, 1.0, 1.0),  # Cyan tint
    7: (1.0, 0.75, 0.5), # Orange tint
    8: (0.75, 1.0, 0.5), # Lime tint
    9: (0.75, 0.5, 1.0), # Purple tint
    10: (0.5, 1.0, 0.75), # Teal tint
    11: (1.0, 0.5, 0.75), # Pink tint
    12: (0.5, 0.75, 1.0)  # Sky Blue tint
}
...
def add_tile_index(image, index_text, tile_size):
    ...
    # Get text bounding box instead of using textsize()
